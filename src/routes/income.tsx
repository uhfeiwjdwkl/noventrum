...
import { incomeBySource, monthlyCashflow, fmtCurrency } from "@/lib/finance/data";
...
function IncomePage() {
  ...
  const sources = incomeBySource(transactions);
  const recent = transactions.filter((t) => t.kind === "income").slice(0, 20);

  const hasIncome = recent.length > 0 || total > 0;

  return (
    <AppShell
      title="Income"
      subtitle="Every dollar earned, from every source."
      actions={<AddTransactionDialog open={addOpen} onOpenChange={setAddOpen} defaultKind="income" trigger={<Button size="sm">Log income</Button>} />}
    >
      {!hasIncome ? (
        <EmptyState
          icon={<ArrowDownCircle className="h-6 w-6" />}
          title="No income logged"
          description="Add salary, dividends, interest or any other earnings."
          action={{ label: "Log income", onClick: () => setAddOpen(true) }}
        />
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            ...
            <Card className="p-5">
              ...
              {sources.length === 0 ? (
                <div className="text-sm text-muted-foreground py-6 text-center">No income sources yet.</div>
              ) : (
                <>
                  <div className="h-56">
                    <ResponsiveContainer>
